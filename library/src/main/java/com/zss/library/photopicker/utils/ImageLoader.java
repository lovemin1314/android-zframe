...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = PhotoUtils.getDiskCacheDir(mContext, "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, StringUtils.getVersionCode(mContext),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    class DiskCacheThread extends Thread {
        ...
        public void run() {
            try {
                String key = PhotoUtils.hashKeyForDisk(path);
